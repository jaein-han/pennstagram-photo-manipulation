...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        int bwH = h + borderWidth;
        int bwW = w + borderWidth;
        ...
        for (int col = 0; col < newW; col++) {
            for (int row = 0; row < newH; row++) {
                if (col < borderWidth || col > bwW - 1 || row < borderWidth || row > bwH - 1) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int newG = 255 - g;
                int newB = 255 - b;
                bmp[row][col] = new Pixel(newR, newG, newB);
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int g = p.getGreen();
                int b = p.getBlue();
                int newR = (int) Math.round(rfactor * r);
                int newG = (int) Math.round(gfactor * g);
                int newB = (int) Math.round(bfactor * b);
                bmp[row][col] = new Pixel(newR, newG, newB);
            }
        }
        return new PixelPicture(bmp);
    ...
    public static PixelPicture alphaBlend(
        ...
        Pixel [][] first = pic.getBitmap();
        Pixel [][] second = f.getBitmap();

        if (wPic == wF && hPic == hF) {
            for (int col = 0; col < wPic; col++) {
                for (int row = 0; row < hPic; row++) {
                    Pixel p1 = first[row][col];
                    Pixel p2 = second[row][col];
                    ...
                    int newR = weightedAverage(alpha, r1, r2);
                    int newG = weightedAverage(alpha, g1, g2);
                    int newB = weightedAverage(alpha, b1, b2);

                    first [row][col] = new Pixel(newR, newG, newB);
                }
            }
        } else {
            return new PixelPicture(first);
        }
        return new PixelPicture(first);
    }
